...
antlrcpp::Any IRGenerator::visitFunctionCall(ifccParser::FunctionCallContext *ctx) 
{
	...
	std::string funcName = ctx->VAR()->getText();
		...
		if (funcName == "getchar" ) {
			...
			while (ctx->expr(i)) { // Check if parameter_list exists
				// Symbol s = it->second;
...
antlrcpp::Any IRGenerator::visitFunctionDef(ifccParser::FunctionDefContext* ctx) {
   ...
   visit(ctx->block());
   string offset = to_string(cfg->functions[funcName].stackOffset);
   //cfg->current_bb->add_IRInstr(IRInstr::ret, INT, {offset});
   function_bb->add_IRInstrAtTop(IRInstr::prologue, INT, {offset});
